            ...
            <form action="{{ route('bookings.store') }}" method="POST">
                ...
                <div class="flex flex-col gap-4">
                    <button type="submit" name="payment_type" value="dp" class="w-full bg-white text-pink-500 font-semibold p-3 rounded-md hover:bg-gray-200">
                        Bayar DP – Rp {{ number_format($product->price / 2, 0, ',', '.') }}
                    </button>
                    <button type="submit" name="payment_type" value="full" class="w-full bg-white text-pink-500 font-semibold p-3 rounded-md hover:bg-gray-200">
                        Bayar Lunas – Rp {{ number_format($product->price, 0, ',', '.') }}
                    </button>
                </div>
            </form>
        </div>
